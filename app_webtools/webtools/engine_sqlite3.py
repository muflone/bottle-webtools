...
from .engine_base import webtoolsEngineBase


class webtoolsEngineSQLite3(webtoolsEngineBase):
    ...
    def list_tables(self):
        ...
        tables = []
        for row in self.get_data('SELECT name '
                                 'FROM sqlite_master '
                                 'WHERE type="table" '
                                 'ORDER BY name')[1]:
            tables.append(row[0])
...
engine_classes = (webtoolsEngineSQLite3, )
